...
public class SSCIssueUpdateCommand extends AbstractSSCJsonNodeOutputCommand implements IRecordTransformer, IActionCommandResultSupplier {
    ...
    @Mixin private SSCCustomTagUpdateMixin.OptionalCustomTagOption customTagsMixin;
    ...
    @Option(names = {"--server-query"})
    @Getter private String serverQuery;
    ...
    public JsonNode getJsonNode(UnirestInstance unirest) {
        ...
        if(selectedIssues.size() > 0){
            SSCBulkResponse bulkResponse = new SSCBulkRequestBuilder()
                    .request("issueAudit", getIssueAuditRequest(unirest, appVersionDescriptor.getVersionId(), selectedIssues))
...
//            .request("updatedVersion", unirest.get(SSCUrls.PROJECT_VERSION(descriptor.getVersionId())))
    ...
    private final HttpRequest<?> getIssueAuditRequest(UnirestInstance unirest, String appVersionId, ArrayNode selectedIssues) {
